...
void creation_arbre_huff(Liste *pile, Liste* Arbre){
    ...
    printf("test 2\n");
            supprimer_pile(pile,&noeud_g); 
            supprimer_pile(pile,&noeud_d); 
            supprimer_file(Arbre,&noeud);

            if((*noeud_d).frequence+(*noeud_g).frequence <= (*noeud).frequence){
    printf("test 3\n");
                printf("%d\n",(*noeud_g).frequence);
                ajouter(Arbre,noeud);
                (*noeud).gauche = noeud_g; a = (*noeud_g).frequence;   
            ...
            else if((*noeud_d).frequence+(*noeud_g).frequence > (*noeud).frequence){
    printf("test 4\n");
                if(size_liste(Arbre)>1){
    printf("test 4.1\n");
                    supprimer_file(Arbre,&noeud2);
                    if((*noeud2).frequence + (*noeud).frequence <= (*noeud_g).frequence + (*noeud_d).frequence){
    printf("test 4.1.1\n");
